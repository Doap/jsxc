...
export function start() {
   ...
   switch (arguments.length) {
      ...
      case 3:
         ...
         break;
      case 4:
         promise = startWithBoshParameters(
            arguments[0],
            arguments[1],
            arguments[2],
            arguments[3]
      ...
      default:
         promise = Promise.reject(
            new InvalidParameterError('Wrong number of parameters.')
         );
   }
...
async function startWithCredentials(
   ...
   let account = await Client.getAccountManager().createAccount(
      url,
      jid,
      password.toString()
   );
...
async function startWithBoshParameters(
   url: string,
   jid: string,
   sid: string,
   rid: string
   ...
   if (!/\/.+$/.test(jid)) {
      return Promise.reject(
         new InvalidParameterError('We need a Jabber ID with resource.')
      );
   }
