...
export default class ChatMarkersPlugin extends AbstractPlugin {
   ...
   constructor(pluginAPI: PluginAPI) {
      ...
      this.pluginAPI.addPreSendMessageStanzaProcessor(
         this.preSendMessageStanzaProcessor
      );

      this.pluginAPI
         .getConnection()
         .registerHandler(this.onChatMarkersMessage, null, 'message');

      this.pluginAPI.registerChatWindowInitializedHook(
   ...
   private hasSubscription(jid: IJID): boolean {
      ...
      return (
         subscription === ContactSubscription.FROM ||
   ...
   private onChatMarkersMessage = (stanza: string) => {
      ...
      let carbonReceivedElement = stanzaElement.find(
         Namespace.getFilter('CARBONS', 'received')
      );
      let carbonSentElement = stanzaElement.find(
         Namespace.getFilter('CARBONS', 'sent')
      );
      let isCarbon =
         carbonReceivedElement.length > 0 || carbonSentElement.length > 0;

      if (
         isCarbon &&
         stanzaElement.attr('from') !==
            this.pluginAPI.getConnection().getJID().bare
      ) {
         this.pluginAPI.Log.warn(
            `Received carbon copy from "${stanzaElement.attr(
               'from'
            )}". Ignoring.`
         );
      ...
      this.pluginAPI.Log.debug(
         `"${marker}" marker received from "${fromAttr}" to "${toAttr}"`
      );
      ...
      if (marker === MARKABLE) {
         ...
         let peerJid = new JID(
            carbonSentElement.length > 0 ? toAttr : fromAttr
         );
         let direction =
            carbonSentElement.length > 0 ? DIRECTION.IN : DIRECTION.OUT;
   ...
   private markMessages(
      markableMessageId: string,
      peer: IJID,
      status: string,
      direction: DIRECTION
   ) {
   ...
   private onChatWindowFocus = (chatWindow: ChatWindow) => {
      ...
      while (!!msg) {
         if (
            msg.getDirection() === DIRECTION.IN &&
            msg.getType() !== Message.MSGTYPE.GROUPCHAT
         ) {
            if (!msg.isDisplayed()) {
