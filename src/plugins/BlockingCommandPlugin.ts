...
export default class BlockingCommandPlugin extends AbstractPlugin {
   ...
   constructor(pluginAPI: PluginAPI) {
      ...
      pluginAPI
         .getSessionStorage()
         .registerHook('blocklist', this.onBlocklistChanged);
      ...
      connection.registerHandler(
         this.onBlocklistUpdate,
         NAMESPACE_BLOCKING_COMMAND,
         'iq',
         'set'
      );
      connection.registerHandler(
   ...
   private async requestBlocklist(): Promise<string[]> {
      ...
      if (!(await this.hasSupport())) {
         this.pluginAPI.Log.info(
            'This server does not support blocking command'
         );
      ...
      let blocklistElement = $(stanza).find(
         `blocklist[xmlns="${NAMESPACE_BLOCKING_COMMAND}"]`
      );
   ...
   public hasSupport(): Promise<boolean> {
      return this.pluginAPI
         ...
         .hasFeature(undefined, NAMESPACE_BLOCKING_COMMAND);
   }
   ...
   private onMessageError = (stanza: string) => {
      if (
         $(stanza).find(`blocked[xmlns="${NAMESPACE_BLOCKING_COMMAND}:errors"]`)
            .length === 0
      ) {
         return true;
      }
      ...
      if (contact) {
         ...
         if (message) {
            message.setErrorMessage(
               Translation.t('You_have_blocked_this_JID.')
            );
