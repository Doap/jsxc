...
export default class Archive {
   ...
   constructor(
      private plugin: MessageArchiveManagementPlugin,
      private contact: Contact
   ) {
      let jid = contact.isGroupChat()
         ? contact.getJid()
         : plugin.getConnection().getJID();
   ...
   private getFirstResultId(): string {
      return (
         this.plugin.getStorage().getItem('firstResultId', this.getId()) || ''
      );
   }
   ...
   private setFirstResultId(resultId: string) {
      return this.plugin
         .getStorage()
         .setItem('firstResultId', this.getId(), resultId);
   }
   ...
   private setExhausted(exhausted: boolean) {
      return this.plugin
         .getStorage()
         .setItem('exhausted', this.getId(), exhausted);
   }
   ...
   public nextMessages() {
      ...
      this.plugin
         ...
         .then(version => {
            if (!version) {
               throw new Error(
                  `Archive JID ${this.archiveJid.full} has no support for MAM.`
               );
            }

            let jid = !this.contact.isGroupChat()
               ? this.contact.getJid()
               : undefined;

            return connection.queryArchive(
               this.archiveJid,
               <string>version,
               queryId,
               jid,
   ...
   public async parseForwardedMessage(
      forwardedElement: JQuery<HTMLElement>
   ): Promise<IMessage> {
      ...
      if (
         this.archiveJid.bare !== from.bare &&
         this.archiveJid.bare !== to.bare
      ) {
         return;
      }
      ...
      let plaintextBody = Utils.removeHTML(
         messageElement.find('> body').text()
      );
      let htmlBody = messageElement.find(
         'html body' + Namespace.getFilter('XHTML')
      );
      ...
      let direction =
         this.contact.getJid().bare === to.bare
            ? Message.DIRECTION.OUT
            : Message.DIRECTION.IN;

      let stanzaIdElement = messageElement.find(
         'stanza-id[xmlns="urn:xmpp:sid:0"]'
      );
      let originIdElement = messageElement.find(
      ...
      if (this.contact.isGroupChat()) {
         ...
         messageProperties.direction =
            contact.getNickname() === from.resource
               ? Message.DIRECTION.OUT
               : Message.DIRECTION.IN;
      }
