...
export default class HttpUploadPlugin extends AbstractPlugin {
   ...
   constructor(pluginAPI: PluginAPI) {
      ...
      pluginAPI.addAfterReceiveMessageProcessor(
         this.extractAttachmentFromStanza
      );
      pluginAPI.addAfterReceiveGroupMessageProcessor(
         this.extractAttachmentFromStanza
      );
   ...
   private preSendMessageProcessor = (
      contact: Contact,
      message: Message
   ): Promise<[Contact, Message]> => {
      ...
      return this.getServices()
         .then(services => {
            ...
            throw new Error(
               'Found no suitable http upload service. File probably too large.'
            );
         })
         .then(service => {
         ...
         .then(downloadUrl => {
            ...
            if (!attachment.setData(downloadUrl)) {
               message.setErrorMessage(
                  Translation.t('Attachment_too_large_to_store')
               );
            }
         })
         .catch(err => {
            ...
            $(stanza)
               ...
               .each((index, element) => {
                  ...
                  let promise = discoInfoRepository
                     ...
                     .then(discoInfo => {
                        let hasFeature = discoInfo.hasFeature(
                           Namespace.get('HTTPUPLOAD')
                        );

                        if (hasFeature) {
                           let maxFileSize = 0;
                           let form = discoInfo.getFormByType(
                              Namespace.get('HTTPUPLOAD')
                           );

                           if (form) {
                              let values =
                                 form.getValues('max-file-size') || [];
                              if (values.length === 1) {
                                 maxFileSize = parseInt(values[0], 10);
                              }
                           }

                           return new HttpUploadService(
                              this.pluginAPI,
                              jid,
                              maxFileSize
                           );
   ...
   private addBitsOfBinary = (
      ...
      if (
         ...
         message.getAttachment().hasThumbnailData()
      ) {
         let attachment = message.getAttachment();
   ...
   private extractAttachmentFromStanza = (
      ...
      let bodyElement = element
         .find('html body[xmlns="' + Strophe.NS.XHTML + '"]')
         .first();
      let dataElement = element.find('data[xmlns="urn:xmpp:bob"]');

      if (
         bodyElement.length &&
         dataElement.length === 1 &&
         !message.isEncrypted()
      ) {
         ...
         if (
            imageElement.length === 1 &&
            'cid:' + cid === imageElement.attr('src')
         ) {
