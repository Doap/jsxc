...
export default class OTRPlugin extends EncryptionPlugin {
   ...
   constructor(pluginAPI: PluginAPI) {
      ...
      pluginAPI.getStorage().registerHook('key', key => {
         if (this.key && this.key !== key) {
            this.pluginAPI.Log.warn(
               'Something went wrong. We have two different DSA keys.'
            );
         }
      ...
      pluginAPI.registerChatWindowInitializedHook((chatWindow: ChatWindow) => {
         ...
         let menuEntry = chatWindow.addMenuEntry(
            'otr-verification',
            'OTR ' + Translation.t('Verification'),
   ...
   private textFormatter = (
      ...
      if (plaintext.match(/^\?OTR([:,|?]|[?v0-9x]+)/)) {
         plaintext =
            ...
            plaintext +
            '">' +
            Translation.t('Unreadable_OTR_message') +
            '</i>';
      }
   ...
   private afterReceiveMessageProcessor = (
      ...
      if (
         !plaintextMessage ||
         (!/^\?OTR/.test(plaintextMessage) &&
            plaintextMessage.indexOf(WHITESPACE_TAG) < 0)
      ) {
