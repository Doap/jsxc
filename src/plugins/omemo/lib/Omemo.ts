...
export default class Omemo {
   ...
   public isTrusted(contact: Contact): boolean {
      ...
      return (
         peer.getTrust() !== Trust.unknown &&
         this.localPeer.getTrust() !== Trust.unknown
      );
   }
   ...
   public async trustOnFirstUse(contact: Contact): Promise<boolean> {
      let peer = this.getPeer(contact.getJid());
      let [
         peerTrustedOnFirstUse,
         localPeerTrustedOnFirstUse,
      ] = await Promise.all([
      ...
      if (peerTrustedOnFirstUse) {
         contact.addSystemMessage(
            Translation.t('Blindly_trusted_peer_on_first_use')
         );
      }

      if (localPeerTrustedOnFirstUse) {
         contact.addSystemMessage(
            Translation.t('Blindly_trusted_all_your_own_devices_on_first_use')
         );
      }
   ...
   public encrypt(
      ...
      return peer
         ...
         .then(encryptedMessages => {
            let stanza = Stanza.buildEncryptedStanza(
               encryptedMessages,
               this.store.getLocalDeviceId()
            );

            $(xmlElement.tree())
               .find(`html[xmlns="${Strophe.NS.XHTML_IM}"]`)
               .remove();
            ...
            xmlElement
               ...
               .t(
                  '***' +
                     Translation.t('You_received_an_OMEMO_encrypted_message') +
                     '***'
               )
               .up();
   ...
   public async decrypt(
      stanza
   ): Promise<{ plaintext: string; trust: Trust } | void> {
      ...
      if (ownPreKeyFiltered.length !== 1) {
         return Promise.reject(
            `Found ${ownPreKeyFiltered.length} PreKeys which match my device id (${ownDeviceId}).`
         );
      }
      ...
      if (authenticationTag.byteLength < 16) {
         ...
         Log.info(
            `Authentication tag is only ${authenticationTag.byteLength} byte long`
         );
      }
      ...
      let ciphertextAndAuthenticationTag = ArrayBufferUtils.concat(
         encryptedData.payload,
         authenticationTag
      );

      return {
         plaintext: await AES.decrypt(
            exportedAESKey,
            iv,
