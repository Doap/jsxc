...
export default class Bundle {
   ...
   public static fromXML(xmlElement): Bundle {
      ...
      let xmlBundle = $(xmlElement).is(targetSelector)
         ? $(xmlElement)
         : $(xmlElement).find(targetSelector);
      ...
      let signedPreKey = new SignedPreKey({
         keyPair: {
            publicKey: ArrayBufferUtils.fromBase64(
               xmlSignedPreKeyPublic.text()
            ),
         },
         signature: ArrayBufferUtils.fromBase64(
            xmlSignedPreKeySignature.text()
         ),
