...
Attachment.registerHandler(
   attachmentHandler.getId(),
...
export default class OMEMOPlugin extends EncryptionPlugin {
   ...
   constructor(pluginAPI: IPluginAPI) {
      ...
      pluginAPI
         .getConnection()
         .getPEPService()
         .subscribe(NS_DEVICELIST, this.onDeviceListUpdate);
      ...
      pluginAPI.addAfterReceiveMessageProcessor(
         this.afterReceiveMessageProcessor
      );
      ...
      pluginAPI.registerChatWindowInitializedHook((chatWindow: ChatWindow) => {
         ...
         chatWindow.addMenuEntry('omemo-devices', 'OMEMO devices', () =>
         ...
         .then(async () => {
            if (!this.getOmemo().isSupported(contact)) {
               if (!(await this.refreshDeviceList(contact))) {
                  throw new Error(
                     Translation.t('Your_contact_does_not_support_OMEMO')
                  );
               }
            }

            if (
               !this.getOmemo().isTrusted(contact) &&
               !(await this.getOmemo().trustOnFirstUse(contact))
            ) {
   ...
   private async refreshDeviceList(contact: IContact) {
      ...
      let stanza = await pepService.retrieveItems(
         NS_DEVICELIST,
         contact.getJid().bare
      );
   ...
   private afterReceiveMessageProcessor = (
      ...
      return this.getOmemo()
         ...
         .then(decrypted => {
            ...
            if (
               decrypted.trust !== Trust.recognized &&
               decrypted.trust !== Trust.confirmed
            ) {
               message.setErrorMessage(
                  Translation.t('Message_received_from_unknown_OMEMO_device')
               );
            }

            if (decrypted.plaintext.indexOf('aesgcm://') === 0) {
               decrypted.plaintext = this.processEncryptedAttachment(
                  decrypted.plaintext,
                  message,
                  contact
               );
   ...
   private processEncryptedAttachment(
      plaintext: string,
      message: IMessage,
      contact: IContact
   ) {
      ...
      if (lines[1] && lines[1].indexOf('data:') === 0) {
         if (
            /^data:image\/(jpeg|jpg|gif|png|svg);base64,[/+=a-z0-9]+$/i.test(
               lines[1]
            )
         ) {
   ...
   private async encryptFile(file: File) {
      ...
      (<any>encryptedFile).aesgcm =
         ArrayBufferUtils.toHex(iv) + ArrayBufferUtils.toHex(keydata);

      return encryptedFile;
   }

   private preSendMessageStanzaProcessor = async (
      message: IMessage,
   ...
   private handleNewDevice(contact: IContact): Promise<void> {
      ...
      overlayElement.append(
         '<p>' + Translation.t('There_are_new_devices') + '</p>'
      );

      let continueButton = $(
         '<button class="jsxc-button jsxc-button--block jsxc-button--primary" />'
      );
      continueButton.text(Translation.t('Configure'));
      overlayElement.append(continueButton);

      let cancelButton = $(
         '<a href="#" class="jsxc-button jsxc-button--block" />'
      );
