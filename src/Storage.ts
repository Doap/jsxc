...
export default class Storage implements IStorage {
   ...
   public static clear(name?: string) {
      Storage.getKeysWithPrefix(name).forEach(key =>
   ...
   public updateItem(key: string, variable: string, value: string): void;
   public updateItem(): void {
      ...
      if (
         arguments.length === 4 ||
         (arguments.length === 3 && typeof variable === 'object')
      ) {
         key = arguments[0] + SEP + arguments[1];
      ...
      if (typeof variable === 'object') {
         $.each(variable, function (key, val) {
            if (typeof data[key] === 'undefined') {
               Log.debug(
                  `Variable ${key} doesn't exist in ${variable}. It was created.`
               );
            }
         ...
         if (typeof data[variable] === 'undefined') {
            Log.debug(
               'Variable ' + variable + " doesn't exist. It was created."
            );
         }
