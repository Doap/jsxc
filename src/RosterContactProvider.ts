...
import {
   ContactSubscription as SUBSCRIPTION,
   IContact,
   ContactType,
...
export default class RosterContactProvider extends ContactProvider {
   ...
   public async add(contact: IContact): Promise<boolean> {
      ...
      await this.getService().addContact(
         contact.getJid(),
         contact.hasName() ? contact.getName() : undefined
      );
   ...
   private processUpdateStanza(stanza: string): boolean {
      ...
      if (rosterVersion) {
         ...
         if (
            ...
            cache.indexOf(contact.getId()) > -1
         ) {
            cache = cache.filter(id => id !== contact.getId());
         } else if (
            subscription !== SUBSCRIPTION.REMOVE &&
            cache.indexOf(contact.getId()) < 0
         ) {
            cache.push(contact.getId());
         }
