...
export default class PluginAPI implements IPluginAPI {
   ...
   public getStorage(): IStorage {
      if (typeof this.storage === 'undefined') {
         this.storage = new Storage(
            this.account.getUid() + ':plugin:' + this.name
         );
      }
   ...
   public addAfterReceiveGroupMessageProcessor(
      ...
      this.account
         .getPipe('afterReceiveGroupMessage')
         .addProcessor(processor, position);
   }
   ...
   public registerConnectionHook(
      func: (status: number, condition?: string) => void
   ) {
   ...
   public registerChatWindowInitializedHook(
      hook: (chatWindow?: ChatWindow, contact?: Contact) => void
   ) {
      this.account.registerChatWindowInitializedHook(hook);
   }

   public registerChatWindowClearedHook(
      hook: (chatWindow?: ChatWindow, contact?: Contact) => void
   ) {
         ...
         (
            ...
            return Promise.resolve(
               formatter(
                  text,
                  direction,
                  this.account.getContact(peer),
                  senderName
               )
            ).then(text => [text, direction, peer, senderName]);
         },
         priority
      );
   }
   ...
   public registerCommand(
      ...
      return this.account
         ...
         .register(command, action, description, category);
   }
