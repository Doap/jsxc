...
export default class IceServers {
   public static isExpiring(): boolean {
      let rtcPeerConfig =
         Client.getOption<IRTCPeerConfig>('RTCPeerConfig') ||
         <IRTCPeerConfig>{};
   ...
   private static getFromUrl(url: string): Promise<ICEServer[]> {
      return new Promise((resolve, reject) => {
         $.ajax(url, {
            ...
            dataType: 'json',
            xhrFields: {
               withCredentials: Client.getOption('RTCPeerConfig')
                  .withCredentials,
            },
         })
            .done((data: IRTCPeerConfig) => {
               let peerConfig: IRTCPeerConfig = Client.getOption(
                  'RTCPeerConfig'
               );
               ...
               if (iceServers && iceServers.length > 0) {
                  let urls =
                     iceServers[0].urls && iceServers[0].urls.length > 0;
            ...
            .fail((xhr, textStatus, error) => {
               Log.warn(
                  'RTC peer config request failed with status: ' + textStatus,
                  error
               );
