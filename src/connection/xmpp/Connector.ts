...
export default class Connector {
   ...
   constructor(private account: Account, ...connectionArgs: string[]) {
      ...
      if (connectionArgs.length < 3) {
         ...
         let type = /^wss?:/.test(connectionArgs[1])
            ? TYPE.WEBSOCKET
            : TYPE.BOSH;
   ...
   public connect() {
      ...
      let isConnectionExpired =
         inactivity &&
         timestamp &&
         new Date().getTime() - timestamp > inactivity;

      if (isConnectionExpired) {
         Log.debug(
            `Inactivity: ${inactivity}, Last timestamp: ${timestamp}, Time diff: ${
         ...
         this.account.triggerConnectionHook(
      ...
      return ConnectHelper.login
         .apply(this, this.connectionArgs)
   ...
   private successfulConnected = data => {
      ...
      let accountConnection = this.replaceStorageConnectionWithXMPPConnection(
         stropheConnection
      );
