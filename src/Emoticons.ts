...
export default class Emoticons {
   ...
   private static shortRegex = new RegExp(
      emojione.regShortNames.source +
         '|(' +
         Object.keys(EMOTICON_LIST.core).join('|') +
         ')',
      'gi'
   );
   ...
   private static replaceShortnameWithImage = (shortname: string) => {
      if (
         ...
         shortname === '' ||
         (!(shortname in EMOTICON_LIST.emojione) &&
            !(shortname in EMOTICON_LIST.core))
      ) {
         return shortname;
      }
