...
export default class JingleSession {
   public static create(account: Account, session: IOTalkJingleSession) {
      ...
      if (sessionType === FILE_TRANSFER_SESSION) {
         throw new Error(
            'We are currently not supporting file transfer sessions.'
         );
         ...
         throw new Error(
            'Could not create jingle session. Unknown session type: ' +
               sessionType
         );
      }
   }
