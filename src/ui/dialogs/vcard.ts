...
export default function (contact: IContact) {
   ...
   for (let resource of resources) {
      let clientElement = dialog
         .getDom()
         .find(`[data-resource="${resource}"] .jsxc-client`);
...
function vcardSuccessCallback(vCardData): Promise<any> {
   ...
   if (
      ...
      (numberOfProperties === 1 && vCardData.PHOTO)
   ) {
      return Promise.reject({});
   }
...
function convertToTemplateData(vCardData): any[] {
   ...
   for (let name in vCardData) {
      ...
      properties.push({
         name: nameLabel,
         value,
         properties: childProperties.filter(
            property => property.value || property.properties.length > 0
         ),
      });
   }
