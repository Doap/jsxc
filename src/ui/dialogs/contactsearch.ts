   ...
   getSearchService(account).then(async services => {
      if (!services || services.length === 0) {
         dialog
            .getDom()
            .find('.jsxc-content')
            .text(Translation.t('not_available'));
            ...
            .map((index, element) => {
               ...
               return discoInfoRepository
                  ...
                  .then(discoInfo => {
                     ...
                     let isMUC = discoInfo.hasFeature(
                  ...
                  .catch(stanza => {
                     ...
                     Log.info(
                        `Ignore ${from} as Search provider, because could not load disco info.`
                     );
...
function generateForm(stanza: Element) {
   ...
   <div class="col-sm-offset-4 col-sm-8">
       ...
       <button class="jsxc-button jsxc-button--primary" type="submit">${Translation.t(
          'Search'
       )}</button>
   </div>
</div>`);
...
function appendSearchResults(resultStanza: Element, dialog: Dialog) {
   ...
   table.find('tr').each((index, row) => {
      ...
      if (index > 0) {
         cell.append(
            `<input type="radio" name="jsxc-add-contact" value="${
               index - 1
            }" required="required"/>`
         );
      }
   ...
   dom.find('.jsxc-results form').on('submit', function (ev) {
      ...
      let selectedIndex = parseInt(
         dom.find('input[name="jsxc-add-contact"]:checked').val().toString(),
         10
      );

      if (!isNaN(selectedIndex)) {
         let extendedItem = $(resultStanza)
            .find('x[xmlns="jabber:x:data"] > item')
            .eq(selectedIndex);
         ...
         if (extendedItem.length === 1) {
            ...
            alias =
               extendedItem.find('field[var="first"]').text() +
               ' ' +
               extendedItem.find('field[var="last"]').text();
         } else if (simpleItem.length === 1) {
            bareJid = simpleItem.attr('jid');
            alias =
               simpleItem.find('first').text() +
               ' ' +
               simpleItem.find('last').text();
         }
      }
...
function generateTableForSimpleResult(items: JQuery) {
   ...
   columnNames.forEach(name =>
      $('<th>').text(Translation.t(name)).appendTo(headerRow)
   );
   ...
   for (let item of items) {
      ...
      columnNames.forEach(name => {
         $('<td>')
            .text($(item).children(name.toLowerCase()).text())
            .appendTo(row);
      });
