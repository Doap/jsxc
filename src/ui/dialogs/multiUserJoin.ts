...
class MultiUserJoinDialog {
   ...
   private updateAccount(id: string) {
      ...
      this.getMultiUserServices().then((services: JID[]) => {
         ...
         services.forEach(service => {
            $('#jsxc-serverlist select').append(
               $('<option>').text(service.domain)
            );
         });
      });
   }
            ...
            $(stanza)
               ...
               .each((index, element) => {
                  ...
                  let promise = discoInfoRepository
                     ...
                     .then(discoInfo => {
                        return discoInfoRepository.hasFeature(
                           discoInfo,
                     ...
                     .catch(stanza => {
                        ...
                        Log.info(
                           `Ignore ${from} as MUC provider, because could not load disco info.`
                        );
                     });
            ...
            return Promise.all(promises).then(results => {
               return results
                  ...
                  .sort(function compare(a, b) {
                     ...
                     if (
                        a.domain.startsWith('conference') &&
                        !b.domain.startsWith('conference')
                     ) {
                        return -1;
                     }
      ...
      return this.connection
         ...
         .catch(stanza => {
            ...
            return Promise.reject(
               'I was not able to get any room information.'
            );
         })
         .then(() => {
            return room;
   ...
   private parseRoomInfo = stanza => {
      ...
      let subject = $(stanza)
         .find('field[var="muc#roominfo_subject"]')
         .attr('label');
      ...
      roomInfoElement.append(
         $('<input type="hidden" name="room-name">').val(name)
      );
      roomInfoElement.append(
         $('<input type="hidden" name="room-subject">').val(subject)
      );
         ...
         .then(stanza => {
            ...
            row.append(
               $('<td>').text(
                  memberJids.length > 0
                     ? memberJids.join(', ')
                     : Translation.t('Occupants_not_provided')
               )
            );
   ...
   private joinHandler = ev => {
      ...
      let name =
         <string>this.dom.find('input[name="room-name"]').val() || undefined;
      let nickname =
         <string>this.nicknameInputElement.val() || this.defaultNickname;
      let password = <string>this.passwordInputElement.val() || undefined;
      let subject =
         <string>this.dom.find('input[name="room-subject"]').val() || undefined;
